 ...
 * <pre>{@code
 * MarkersLogger.createInstance(System.currentTimeMillis(), new MarkersLogger.ReactContextProvider() {
 *       @Override
 ...
 * Add this: {@code require('react-native/Libraries/wix/markersLogger')} at the earliest-executed point of your JS code.
 *
 ...
 * <br/>(exact command should appear in the device logs with a {@code RNMARKERS_LOGGER} prefix).
 * <br/>If you're having troubles running adb, try this <a href="https://developer.android.com/studio/command-line/adb">info page</a>.
 */
public class MarkersLogger {
  ...
  protected static class UnifiedRecord {
    ...
    UnifiedRecord(String name, String tag, int instanceKey) {
    ...
    UnifiedRecord(String name, String tag, int instanceKey, long time) {
      mTime = time;
      mName = name;
      mTag = tag;
      mInstanceKey = instanceKey;
  ...
  private final List<UnifiedRecord> mLog = new LinkedList<>();
  ...
  protected void logCustomMarker(String name, String tag, Long time) {
    mLog.add(new UnifiedRecord(name, tag, -1, time));
  }
    ...
    ReactMarker.addListener(
            new ReactMarker.MarkerListener() {
              ...
              public void logMarker(ReactMarkerConstants name, @Nullable String tag, int instanceKey) {
                mLog.add(new UnifiedRecord(name.toString(), tag, instanceKey));
              }
            });
  }
  ...
  protected String createLogsJSON(long startTime, @Nullable List<UnifiedRecord> records) {
    final JSONObject result = new JSONObject();
    try {
